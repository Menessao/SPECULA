    ...
    for seeing in seeings:
        for r_val in r_vals:
            for starMag in starMags:  
                freq = 1000 if starMag < 9 else 200  
                ...
                store_dir = f'{dirname}/r{r_val:.2f}'
                overrides = ("{"
                        ...
                        f"filter.int_gain: [1.0], "
                        f"filter.delay: {delay*freq:1.2f}, "
                        f"omgi.r: {r_val:1.2f}, "
                        f"omgi.dt: {(2*(delay*freq+1)-1):1.2f}, "
                        f"data_store.store_dir: {store_dir}, "
                        f"data_store.create_tn: false, "
                        f"data_store.inputs.input_list: ['sr-psf.out_sr', 'opt_gains-omgi.out_gains',]"
                        "}")
                ...
                results.append({'seeing': seeing,'freq': freq,'starMag': starMag, 'sr': sr, 'sr_std': sr_std, 'filter': filtertype, 'r': r_val})
                columns = ['seeing', 'freq', 'starMag', 'sr', 'r_val', 'filter']

    results_df = pd.DataFrame(results, columns=columns) 
    results_df.to_csv(os.path.join(result_dir, 'e2e_csv', f'rMod{rMod:1.0f}_r{r_val:1.2f}_{nSubap}x{nSubap}_{nModes}modes.csv'), index=False)
